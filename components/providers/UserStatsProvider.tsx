...
export function UserStatsProvider({children}: {children: React.ReactNode}) {
  ...
  const loseHeart = useCallback(async () => {
    try {
      ...
      setStats(prev => (prev ? {...prev, hearts: Math.max(0, prev.hearts - 1)} : prev))
    }
  }, [fetchApi])
  ...
  return <UserStatsContext.Provider value={{stats, loading, refresh, loseHeart}}>{children}</UserStatsContext.Provider>
}
